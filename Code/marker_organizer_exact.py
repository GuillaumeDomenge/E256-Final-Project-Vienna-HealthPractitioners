...
with tqdm(total=23) as p_bar:
    for d in districts:
        l= list(data[d])
        dists = np.array([[100000*distance(l[i],l[j]) for i in range(len(l))] for j in range(len(l))])
        sol , bined = solve_tsp_brute_force(dists)
        l2 = [l[i] for i in sol]
        newdata[d]=l2
        p_bar.update(1)
...
with open('../Data/Json/OrderedMarkers2', 'w') as fout:
